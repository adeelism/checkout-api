...
"""
Calculates the total for the coming product list
by comparing against the catalogue and applying any bundle offers if applicable
"""
...
def calculate_total(product_ids: list[str]) -> int:
    ...
    total = sum(
        price_for_quantity(quantity, CATALOGUE[product_id])
        for product_id, quantity in quantities.items()
    )
    return total


"""helper function to determine if there is an offer on the
product or not an offer on the product and calculate the total price accordingly"""
